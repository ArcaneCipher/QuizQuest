...
function requireLogin(req, res, next) {
  if (!req.session || !req.session.userId) {
    if (req.originalUrl === '/quizzes/new') {
      return res.status(401).json({ error: 'You must log in to create a quiz.' });
    }
    return res.redirect('/'); // Redirect to homepage for other routes
  }
  next();
}
